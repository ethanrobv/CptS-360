    ...
    while (nbytes)
    {
        ...
        else
        {
            ...
            if (!running->fd[fd]->minodePtr->INODE.i_block[13])
            {
                ...
                bzero(tbuf, sizeof(tbuf));
                put_block(running->fd[fd]->minodePtr->dev, running->fd[fd]->minodePtr->INODE.i_block[13], tbuf);
            }
            ...
            int tbuf[256];
            int ttbuf[256];
            //get double indirect block into tbuf
            get_block(running->fd[fd]->minodePtr->dev, running->fd[fd]->minodePtr->INODE.i_block[13], tbuf);
            int dblk = tbuf[lblk / 256];
            //if dblk does not exist, must be allocated
            //and recorded in the INODE, should be initialized to 0
            if (!dblk)
            {
                tbuf[lblk / 256] = balloc(running->fd[fd]->minodePtr->dev);
                dblk = tbuf[lblk / 256];
                int kbuf[256];
                bzero(kbuf, sizeof(kbuf));
                put_block(running->fd[fd]->minodePtr->dev, dblk, kbuf);
            }

            //if physical block doesn't exist, must be allocated
            get_block(running->fd[fd]->minodePtr->dev, dblk, ttbuf);
            blk = ttbuf[lblk % 256];
            if (!blk)
            {
                ttbuf[lblk % 256] = balloc(running->fd[fd]->minodePtr->dev);
                blk = ttbuf[lblk % 256];
                put_block(running->fd[fd]->minodePtr->dev, dblk, ttbuf);
            }
        }
